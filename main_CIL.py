...
from models.model import Baseline_CNN, BCResNet_Mod,Audio_Encoder
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', type=str, default='Audio_Encoder')  # 'baseline' | 'BC-ResNet'
    ...
    parser.add_argument(
        ...
        type=str,
        default='uncertainty',
        help="memory management [random, uncertainty, reservoir, prototype,MIO]",
    )
    parser.add_argument("--num_pretrain_class", type=int, default=None,
                        help="The number of classes in pretrained model")
    parser.add_argument("--n_tasks", type=int, default=5, help="The number of tasks")
    parser.add_argument(
        "--n_cls_a_task", type=int, default=13, help="The number of class of each task"
    )
    parser.add_argument(
        "--n_init_cls",
        type=int,
    ...
    mem_manage =  args.mem_manage

    # Log file initalization
    ckpt_dir = os.path.join('workspace', dataset, exp_name, mem_manage,'save_models')
    os.makedirs(ckpt_dir, exist_ok=True)
    ckpt_name = os.path.join(ckpt_dir, 'last.pt')
    # ckpt_path = ckpt_name if os.path.exists(ckpt_name) else None

    log_dir = os.path.join('workspace', dataset, exp_name, mem_manage,'logs')
    ...
    num_class = None
    num_pretrain_class = None
    if args.load_from_ckpt is not None:
        assert args.num_pretrain_class is not None
        num_pretrain_class = args.num_pretrain_class
    if dataset == 'ESC-50' or dataset == 'Audioset' or dataset =='ref_youtube_audio':
    ...
    loss_func = get_loss_func('focal_loss')
    if model_name == 'baseline':
        model = Baseline_CNN(num_class=num_class if num_pretrain_class is None else num_pretrain_class,
                             frontend=frontend)
    elif model_name == 'BC-ResNet':
        model = BCResNet_Mod(num_class=num_class if num_pretrain_class is None else num_pretrain_class,
                             frontend=frontend)
    elif model_name == 'Audio_Encoder':


        feature_extractor = AutoFeatureExtractor.from_pretrained(
            "/home/user/SED_Adaptation_Classifier-main/whisper_model")

        # whisper_model = WhisperForAudioClassification.from_pretrained("/home/user/SED_Adaptation_Classifier-main/whisper_model")
        whisper_model = whisper.load_model("small")
        print("This is whisper_model")
    ...
    if args.load_from_ckpt is not None:
        model.load_state_dict(torch.load(args.load_from_ckpt))
        model.add_new_outputs(num_class)
    ...
    for cur_iter in range(args.n_tasks):
        ...
        if args.dataset == "ESC-50":
            ...
            cur_train_datalist = get_train_datalist(args, cur_iter)
            cur_test_datalist = get_test_datalist(args, args.exp_name, cur_iter)
            # cur_train_datalist = get_train_datalist(args, 4)
            # cur_test_datalist = get_test_datalist(args, args.exp_name, 4)
            ...
            logger.info(f"[2-3] Start to train")
            if cur_iter > 0:
                path = '/home/user/SED_Adaptation_Classifier-main/workspace/ref_youtube/{}/iter{}epoch.pt'.format(method.mem_manage,cur_iter-1)
                method.change_model(path)
            task_acc,task_stat = method.train(
                n_epoch=args.epoch,
                batch_size=args.batch_size,
                n_worker=8,
                iteration=cur_iter
        ...
        task_stats.append((cur_iter,task_stat))
    ...
    np.save(f"/home/user/SED_Adaptation_Classifier-main/workspace/ref_youtube/finetune/{save_path}.npy", task_records)
    # Total time (T)
    ...
    logger.info(f'task_stats time {task_stats}')
